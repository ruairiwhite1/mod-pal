...
module.exports = {
    ...
    dmEmbed = `☹️ You've been muted in **${message.guild.name}** for **${msRegex.exec(args[1])[1]}**`
    ...
    mentionedMember.send(new MessageEmbed().setColor('RED')
    .setDescription(dmEmbed)
    ).catch(()=>{})
    }
        message.channel.send(new Discord.MessageEmbed().setColor('GREEN').setDescription(`🎉 | Muted **${mentionedMember.user.tag}** for **${msRegex.exec(args[1])[1]}** ${logging && logging.moderation.include_reason === "true" ?`\n\n**Reason:** ${reason}`:``}`)).then(async(s)=>{
